    ...
    def load_pedidos(self, path: str, compression: str, header: bool, sep: str) -> DataFrame:
        try:
            return (self.spark.read.format("csv")
                .option("compression", compression).option("header", header)
                .option("sep", sep).option("encoding", "UTF-8")
                .option("mode", "FAILFAST").schema(self._get_schema_pedidos()).load(path))
    ...
    def load_pagamentos(self, path: str) -> DataFrame:
        try:
            return (self.spark.read.format("json")
                .option("mode", "FAILFAST").schema(self._get_schema_pagamentos()).load(path))
        except AnalysisException as e:

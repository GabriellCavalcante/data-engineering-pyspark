...
class Transformation:
    ...
    def gerar_relatorio(self, pedidos_df: DataFrame, pagamentos_df: DataFrame) -> DataFrame:
        ...
        try:
            ...
            pedidos = (
                pedidos_df.withColumn("data_pedido", F.coalesce(
                    F.to_timestamp("data_criacao", "yyyy-MM-dd'T'HH:mm:ss"),
                    F.to_timestamp("data_criacao", "yyyy-MM-dd HH:mm:ss"),
                    F.to_timestamp("data_criacao", "yyyy-MM-dd")))
                .withColumn("uf", F.upper(F.trim("uf")))
                .filter(F.col("id_pedido").isNotNull() & F.col("data_pedido").isNotNull())
                .filter((F.col("data_pedido") >= F.lit("2025-01-01").cast("timestamp")) &
